...
- **Homebrew は、入れた直後は PATH に無い。** Apple Silicon は `/opt/homebrew/bin/brew`、Intel は `/usr/local/bin/brew`。使うコマンドと同じ呼び出しの頭で `eval "$(<brew のパス> shellenv)"` を読み込む。インストーラが案内する `~/.zprofile` への追記は、SKILL.md の決まり5のとおりステップ6で扱う。
- **Homebrew のインストーラには sudo が要る。** SKILL.md の決まり8のとおりに用意する。端末の無い呼び出しでは非対話で動き、sudo が通らないと中止する（確かめてから）。Xcode Command Line Tools が無ければ一緒に入り、git も使えるようになる（確かめてから）。
...
- **GUI アプリが apt に無ければ、flatpak か snap で入れる**（WSL では GUI アプリを入れない）。
- **apt がロックで失敗したら、自動更新（unattended-upgrades など）が終わるのを待って再試行する。** ロックの持ち主は `sudo lsof /var/lib/dpkg/lock-frontend` で見る。ロックファイルは消さずに待つ（消すと動いている更新を壊す）。中断した dpkg は `sudo dpkg --configure -a` で直す。
- **Docker Engine を入れたなら、docker グループへの追加（sudo が要る）は再ログインするまで効かない。**
- **WSL の `/etc/wsl.conf` の変更は、Windows 側で `wsl --shutdown` するまで効かない。** 打つと WSL の中の Claude Code も止まるので、ステップ7に回す。
...
- **設定ファイルを symlink で展開するには、開発者モードか管理者権限が要るはず（確かめてから）。**
- **Windows PowerShell 5.1 でスクリプトやプロファイルが止まったら、実行ポリシーを確かめる（確かめてから）。** PowerShell 7 の既定は RemoteSigned。
