...
指定すれば、Claude Code の更新の知らせ（再起動で反映される）や、放置から戻ったときのヒント（`/clear` の勧め）が出ているセッションも知らせる。
知らせを受けたセッションが、上と同じ手順で handoff を依頼する。区切りの判断は対象に残る。
既定では、知らせを受けると確認なしで再起動の手順に進む。知らせのたびにユーザーに確認させることもできる。

このリポジトリで起動すると、見張りを始めさせる（`.claude/settings.json` の SessionStart hook。中身は SKILL.md の「コンテキストを見張る」）。
