...
- **このリポジトリで起動すると、コンテキストの見張りが自動で始まり、知らせは確認なしで流す。** 見張るのは Herdr 上の claude ペインすべて（他のプロジェクトのものも）。使用率が閾値（既定 30%）以上になるか更新の知らせが出たセッションは会話ログを引き継いで再起動し、放置のヒントが出たセッションには `/clear` を送る。知らせのたびに確認させたいなら、記憶機能か `CLAUDE.local.md` に書く
