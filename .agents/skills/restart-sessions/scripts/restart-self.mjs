...
3. ユーザーに再起動の結果を報告し、AskUserQuestion で「お任せで進める」「何もしない」の二択を出す。答えが返るまで作業を始めないこと。「お任せで進める」が選ばれたら、資料の残りから次の一手を選んで進めます。ただし後戻りできない操作と外向きの操作（push・PR・マージ・削除・他セッションへの依頼）は、そのつど確認してください。`;
