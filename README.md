...
- **再起動した先は、資料に書かれた前の状態から続ける。** 作業の途中なら再開し、ユーザーの答えを待っていたなら同じ問いを出し直して待つ。再起動を知らせる問いは出ない（呼び出し元が報告する）。後戻りできない操作や外向きの操作（push・マージ・削除など）は、前のセッションで承認済みでも改めて確認させる
- **前の状態の見分けは、handoff の skill が資料の冒頭に状態を書くことに頼っている。** 書かれていなければ、新セッションはユーザーに聞いて止まる
...
- 選択肢で答える問い（AskUserQuestion）が使える（再起動の前に選択式の問いで待っていたとき、新セッションが出し直す）
