...
- **再起動した先は、必ず進め方の二択（お任せで進める／何もしない）を出して止まる。** 進められる作業があっても、まず聞く。問いが出ること自体が、再起動に気づく手がかりになるため。答えるまでそのペインは待ち状態で、見張りからは読めず、依頼も届かない
...
- `@<path>` で資料が読み込まれる
- 選択肢で答える問い（AskUserQuestion）が使える（再起動の後の二択）
...
- `herdr agent wait --until` が指定した状態になるまで待ち、時間切れなら `timeout` のエラーを返す（再起動の後に、資料の依頼が送信されたかの確認）
