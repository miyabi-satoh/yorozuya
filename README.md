...
- **再起動した先は、資料に書かれた前の状態から続ける。** 作業の途中なら再開し、ユーザーの答えを待っていたなら同じ問いを出し直して待つ。再起動したことを知らせる問いは出さないので、気づくのは画面を見たときになる。後戻りできない操作や外向きの操作（push・マージ・削除など）は、前のセッションで承認済みでも改めて確認させる
