...
- **再起動した先は、資料に書かれた前の状態から続ける。** 作業の途中なら再開し、ユーザーの答えや操作を待っていたなら同じ問いや依頼を出し直して待つ。再起動を知らせる問いは出ない（再起動したことは呼び出し元が、自己再起動では新セッションが報告する）。後戻りできない操作や外向きの操作（push・マージ・削除など）は、前のセッションで承認済みでも改めて確認させる。ただし、CLAUDE.md や記憶で前もって決めた扱いがあれば、それに従う
