...
- **見張りは、見張る側のセッションが起きている間だけ動く。** そのセッションの Monitor の中で走る。Monitor は最長30分で切れるので、そのたびに張り直させる。セッションを閉じれば見張りも止まる
...
hook からは Monitor を呼べないので、見張りが始まるのはそのセッションが最初に返事をするとき。matcher を `startup` にした hook は、再開（resume）したセッションでは動かなかった。
